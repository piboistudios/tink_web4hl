...
TestServer.__name__ = true;
TestServer.noop = function() {
	return;
};
TestServer.main = function() {
	...
	container.run(new tink_http_SimpleHandler(function(req) {
		...
		return ret.gather();
	})).handle(tink_core__$Callback_Callback_$Impl_$.fromNiladic(TestServer.noop));
	console.log("src/TestServer.hx:25:","Listening at http://localhost:8080");
};
var Root = function() {
};
Root.__name__ = true;
Root.prototype = {
	large_file: function(body) {
		console.log("src/TestServer.hx:34:","Receiving large_file");
		var output = new haxe_io_BytesOutput();
		var outSink = tink_io__$Sink_SinkYielding_$Impl_$.ofOutput("some-new-sink",output);
		var response = new tink_core_FutureTrigger();
		tink_io__$Source_Source_$Impl_$.pipeTo(body,outSink).handle(function(res) {
			var text = output.getBytes().toString();
			response.trigger(text);
			js_node_Fs.writeFileSync("./response.out",text);
			return;
		});
		return response;
	}
	,hello: function() {
		console.log("src/TestServer.hx:52:","Hello");
		return "Hello!";
	}
	,__class__: Root
};
...
tink_web_routing_Context.prototype = {
	...
	,headers: function() {
		...
		while(f.hasNext()) {
			var f1 = f.next();
			_g.push(new tink_core_NamedWith(f1.name,f1.value));
		}
...
tink_web_routing__$Response_Response_$Impl_$.ofRealSource = function(source) {
	var this1 = httpstatus__$HttpStatusMessage_HttpStatusMessage_$Impl_$.fromCode(200);
	var this2 = new tink_http_ResponseHeaderBase(200,this1,[new tink_http_HeaderField("content-type","application/octet-stream")],"HTTP/1.1");
	var this3 = new tink_http__$Response_OutgoingResponseData(this2,tink_io_RealSourceTools.idealize(source,function(_) {
		return tink_io__$Source_Source_$Impl_$.EMPTY;
	}));
	return this3;
};
tink_web_routing__$Response_Response_$Impl_$.ofIdealSource = function(source) {
	var this1 = httpstatus__$HttpStatusMessage_HttpStatusMessage_$Impl_$.fromCode(200);
	var this2 = new tink_http_ResponseHeaderBase(200,this1,[new tink_http_HeaderField("content-type","application/octet-stream")],"HTTP/1.1");
	var this3 = new tink_http__$Response_OutgoingResponseData(this2,source);
	return this3;
};
	...
	route: function(ctx) {
		...
		case "GET":
			if(_g1 == false) {
				...
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this1.query == null ? this1.path : this1.path + "?" + this1.query),{ fileName : "src/TestServer.hx", lineNumber : 21, className : "tink.web.routing.Router0", methodName : "route"}))));
			}
			break;
		case "POST":
			if(_g2 == "large-file") {
				if(_g1 == true) {
					if(_g == false) {
						return tink_core__$Promise_Promise_$Impl_$.ofSpecific(this.large_file(ctx));
					} else {
						var this2 = ctx.request.header.url;
						return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this2.query == null ? this2.path : this2.path + "?" + this2.query),{ fileName : "src/TestServer.hx", lineNumber : 21, className : "tink.web.routing.Router0", methodName : "route"}))));
					}
				} else {
					var this3 = ctx.request.header.url;
					return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this3.query == null ? this3.path : this3.path + "?" + this3.query),{ fileName : "src/TestServer.hx", lineNumber : 21, className : "tink.web.routing.Router0", methodName : "route"}))));
				}
			} else {
				var this4 = ctx.request.header.url;
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this4.query == null ? this4.path : this4.path + "?" + this4.query),{ fileName : "src/TestServer.hx", lineNumber : 21, className : "tink.web.routing.Router0", methodName : "route"}))));
			}
			break;
		default:
			var this5 = ctx.request.header.url;
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this5.query == null ? this5.path : this5.path + "?" + this5.query),{ fileName : "src/TestServer.hx", lineNumber : 21, className : "tink.web.routing.Router0", methodName : "route"}))));
		}
	}
	,large_file: function(ctx) {
		var body;
		...
		if(_g._hx_index == 0) {
			...
			body = tink_io__$Source_Source_$Impl_$.ofError(new tink_core_TypedError(501,"not implemented",{ fileName : "tink/web/routing/Context.hx", lineNumber : 47, className : "tink.web.routing.Context", methodName : "get_rawBody"}));
		}
		var ret = this.target.large_file(body).map(tink_core_Outcome.Success);
		return tink_core__$Promise_Promise_$Impl_$.next(ret.gather(),tink_core__$Promise_Next_$Impl_$.ofSafeSync(function(v) {
